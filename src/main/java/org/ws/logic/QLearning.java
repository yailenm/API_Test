...
import org.ws.gui.Instance;
import org.ws.gui.MachineGUI;
import org.ws.gui.OperationAllocation;
import org.ws.gui.OperationGUI;
//import org.ws.gui.Pair;
import org.ws.gui.Schedule;
//import org.ws.gui.ScheduleFrame;
import org.ws.gui.Instance;
...
public class QLearning {
	...
	//public int lastJob = 1; //ultimo trabajo procesado
					...
					if (last_op.GetJob() == jobsInExecute[0]) {//colocar en 0 porque el trabajo ya termino
						...
						for (Job job : Jobs) {
							if (!job.finished && job.GetID() != jobsInExecute[1]) {
								job.Start(Machines);
								//System.out.println("posicion 0 envie "+Jobs[i].GetID());
							}
						}
					} else {
						jobsInExecute[1] = -1;
						for (Job job : Jobs) {
							if (!job.finished && job.GetID() != jobsInExecute[0]) {
								job.Start(Machines);
								//System.out.println("posicion 1 envie "+Jobs[i].GetID());
							}
						}
...
//				}
	...
	public void ExecuteReSchedule() throws FileNotFoundException, CloneNotSupportedException{
		...
		//for (int n = 0; n < 1; n++){
			...
			jobsInExecute[1] = -1;
			for (int i = 0; i < Jobs.length; i++) {
				if(Jobs[i].opStart > 0 && Jobs[i].opStart < Jobs[i].operations.size() 
...
//	 printValues();
	...
	/*public void SearchRoutesVersionNewProducts(int cantNewProd){	
		int j_index=0;
		//HabOperationList.clear();
		//Digo que las operaciones iniciales de cada trabajo estan habilitadas		
		for(int j=Jobs.length-cantNewProd; j < Jobs.length; j++)
			HabOperationList.add(Jobs[j].operations.get(0));//solo se agregan la primera op de cada job
		...
		while (!HabOperationList.isEmpty()){
			...
			for (int i = o_index+1; i < Jobs[j_index].operations.size(); i++) {//por si hay mas de dos op seguidas en un mismo job
				//if (Jobs[j_index].operations.length-1 != o_index){
				//si existe backtoback
					if (Jobs[j_index].operations.get(i).back2back_before == o_index) {
						 Oper = Jobs[j_index].operations.get(i);
						 FullOperationList.add(Oper);//se agrega la op
						// HabOperationList.remove(i);//y se elimina de la lista
						 o_index = Oper.GetID();
						 break;
					}else {
						//si llega al final y no encuentra backtoback
						if (i == Jobs[j_index].operations.size()-1) {
							//si Oper no es la ultima del trabajo se agrega la siguiente op a la lista
							if (Jobs[j_index].operations.size()-1 != o_index){
								HabOperationList.add(Jobs[j_index].operations.get(o_index+1));
							}
